    ...
    return (
        ...
        >
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
                {/* Cover Art */}
                <div className="aspect-square w-full rounded-xl bg-[#282828] overflow-hidden shadow-2xl">
                    {thumbnailUrl ? (
                ...
                <div className="space-y-1">
                    <div className="flex items-center justify-between gap-2">
                        <h4 className="text-[20px] font-bold text-white line-clamp-2 leading-tight">
                            {song?.title || "No track selected"}
                        </h4>
                        <Heart className="w-5 h-5 text-neutral-400 hover:text-[#1DB954] cursor-pointer transition-colors flex-shrink-0" />
                    </div>
                            ...
                            return (
                                <div key={s.id} className="flex items-center gap-3 group">
                                    ...
                                    <div className="min-w-0">
                                        <p className={`text-[13px] font-medium truncate ${song?.id === s.id ? 'text-[#1DB954]' : 'text-white'}`}>{s.title}</p>
                                        <p className="text-[11px] text-neutral-500 truncate">{(s as any).channel_title || 'YouTube'}</p>
                                    </div>
                                </div>
                            );
                        })}
                ...
                {/* Source Info */}
                <div className="bg-[#181818] rounded-xl p-4 space-y-2">
                    <h5 className="text-[13px] font-bold text-white">Source</h5>
